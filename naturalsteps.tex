...
\def\pictitle{A little girl and a cow}
\def\picauthor{Yoshio Yamamoto}

% Japanese用の条件マクロ \ifJPN が完了したら、削除すること。
\newif\ifJapanese
% 英語のスイッチ
%\Japanesefalse
\Japanesetrue
...
\fi
\ifJPN
おそらく「自然な話し方」を達成するためには、2つの自然なアプローチが必要である。
1つは、会話自体が自然なことと、学び方が自然であること。
\else
...
\fi
\ifJPN
「自然な話し方」を身に着けるには、は従来、文型、語順というルールを一旦、忘れ、現実の語順を十分に観察すること。
\else
To acquire {\it Natural speaking\/}, it is necessary to forget the rules of grammar and word order for a while and observe the actual word order carefully.
\fi
\ifJPN
一方、句の中の語順は厳格で、動詞の活用は正確でなければならない。
そのルールを守らないと、意味が大きく変わってしまう。
\else
On the other hand, the word order in phrases is strict, and verb conjugations must be accurate.
If you do not follow these rules, the meaning will change significantly.
\fi
\ifJPN
...
\ifJPN
話す技術は少しずつだんだん上手になっていくものなのに、初級の教科書を見ると、とうてい覚えられない長い文が紹介されている。
\else
Speaking skills gradually improve little by little, but when you look at beginner textbooks, you will find long sentences that you can't remember.
\fi
\ifJPN
もちろん、そんなに長い文、覚えられないし、言えるはずもない。
\else
Of course, you can't remember or say such long sentences.
\fi
\ifJPN
会話が難しいとか発音が難しいとかは、学ぶ側の問題ではなく、教える側の問題である。
\else
The difficulty of conversation or pronunciation is not a problem on the learner's side, but on the teacher's side.
\fi
\ifJPN
本当に口を開くとはどういうことなのか、その現実を教育者が分析していないから、学生は不安になり、話せなくなる。
\else
Because educators have not analyzed what it really means to open their mouths, students become anxious and unable to speak.
\fi
\ifJPN
教師の会話場面の分析不足は、教師が言語学習初期の学生を子供のように扱うことからも推測できる。
\else
The lack of analysis of the teacher's conversation scene can be inferred from the fact that teachers treat students in the early stages of language learning like children.
\fi
\ifJPN
短い発話は、子供っぽい言い方ではない。
\else
Short utterances are not childish expressions.
\fi
\ifJPN
ある教師は、短い発話は子供っぽいと言っていた。
\else
One teacher said that short utterances are childish.
\fi
\ifJPN
 「たべる」「くる」「うん、いく」「まだ、しない」などの短い発話は、子供っぽいということらしい。 
\else
Short utterances such as ``eat,'' ``come,'' ``yes, go,'' and ``not yet, not do'' seem to be considered childish.
\fi
\ifJPN
しかし、これは間違い。
大人であっても、このような短い発話を日常的に使っている。
\else
However, this is a mistake.
Even adults use such short utterances in everyday life.
\fi
\ifJPN
...
子供っぽさは内容にある。
\else
There are many communications expressed briefly in an adult context.
It is not the way of speaking that is childish.
Childishness is in the content.
\fi
\ifJPN
...
\else
Learn from frequently used verbs and frequently used forms.
Not all conjugations are used with the same frequency, so you don't need to remember all the conjugations.
\fi
\ifJPN
たとえば、「買う」の否定形を覚えるのではなく、「買わない」を覚え、練習する。
\else
For example, instead of learning the negative form of ``kau (buy),'' learn and practice ``kawanai (not buy).''
\fi
\ifJPN
単に頻度が高いからという理由で学ぶのではなく、覚えやすい順番と覚え方の技術を身につけること。
\else
Learn not just because it is frequently used, but also learn the order and method of memorization that is easy to remember.
\fi
\ifJPN
すぐに使える形を見つけ、忘れる前にすぐ使う。これを繰り返す。
\else
Find a form that you can use immediately and use it right away before you forget it. Repeat this.
\fi
...
\else
Look up the Japanese words for ``go,'' ``come,'' ``eat,'' ``is,'' and ``do.''
