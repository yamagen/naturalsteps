...
\begin{abstract}
\ifEnglish
There are not much about things to need to know before practicing.
\else
練習する前に知っておかなければならないことはあまりない。
\fi
\ifEnglish
Students should concentrate on practicing actual conversation and writing.
\else
学生は実際の会話や作文に集中して練習すればよい。
\fi
\ifEnglish
However, it is better to learn beforehand how to continue the actual practice and the minimum structure of the language.
\else
ただし、あらかじめ、実際の練習を長続きさせるための工夫や最低限の言語の仕組みを学んでおいたほうが良い。
\fi
\end{abstract}
...
\ifEnglish
Pay attention to the structure of the language of Japanese and the flow of conversation.
\else
日本語という言語の構造と会話の流れに注目する。
\fi
\ifEnglish
...
\fi
\ifEnglish
Subjects may be omitted as long as they are understood in context.
\else
主語は文脈でわかる限り、省略しても良い。
\fi
\ifEnglish
...
\fi
\ifEnglish
In any language, the word order is quite free, and in conversation, many words can be omitted as long as both parties are understood.
\else
どの言語でも、語順はかなり自由であり、会話にて、話者や動作主がわかれば、多くの語を省略できる。
\fi
\ifEnglish
Therefore, you can speak in any order you feel comfortable with.
\else
だから、思った順に安心して話しても良い。
\fi
\ifEnglish
This does not mean that you are free to change the order of all words or omit any word you like.
\else
ただ、まったく自由にすべての語の順序を変えてもよい、自由に好きな語を省略しても良い、というわけではない。
\fi

\begin{toiquestion}
 \ifEnglish
 When you speak your mother tongue, think about whether you can omit the subject.
 \else
...
If only the most important words are uttered and the meaning is not ambiguous, the meaning can be understood in one word.
If ambiguous, add words to disambiguate.
Whether it is a subject or an object depends on the context.
Whether or not it can be omitted depends on the context, but in many cases it should.
\else
どの言語でも、一語の発話が見られる。
最も大切な語だけを発話して、意味が曖昧でなければ、一語で意味がわかる。
もし、曖昧なら、語を追加して、曖昧さを解消する。
それが主語であるか、目的語であるかは、そのときの文脈による。
省略してもよいかどうかは文脈によるが、多くの場合、省略してもよいはずだ。
\fi
\end{toianswer}
